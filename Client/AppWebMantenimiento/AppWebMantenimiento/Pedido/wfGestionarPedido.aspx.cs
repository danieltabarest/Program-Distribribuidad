...
namespace AppWebMantenimiento.Pedido
{
    public partial class wfGestionarPedido : System.Web.UI.Page
    {
        ...
        clsDistribPedido objDistribPed;
        ...
        /// <summary>
        /// Metodo llamando la consolo de distribuidad
        /// </summary>
        private void GrabarPedidoRem()
        {
            lblMsjCab.Text = "";

            objDistribPed = new clsDistribPedido();

            if (String.IsNullOrEmpty(txtNumOrd.Text))
            {
                objDistribPed.gsNroOrd = 0;
            }
            else
            {
                objDistribPed.gsNroOrd = Convert.ToInt32(txtNumOrd.Text);
            }
            objDistribPed.gsFecOrd = calFecOrd.SelectedDate;
            objDistribPed.gsCodCli = ddlCliente.SelectedValue;
            objDistribPed.gsCodTec = ddlTecnico.SelectedValue;
            objDistribPed.gsVlr = Convert.ToDecimal(txtValor.Text);
            objDistribPed.gsIva = Convert.ToDecimal(txtIva.Text);

            objDistribPed.gsDtDetalle = (DataTable)Session["varDtDetalle"];

            if (objDistribPed.GrabarPedidoRemoto())
            {
                lblMsjCab.Text = "Orden # " + objDistribPed.gsNroOrd.ToString() + " Grabada Exitosamente";
                BloquearCamposGral(false);
                LimpiarCampDet();
                LimpiarCabOrdComp();
                txtNumOrd.Enabled = true;
            ...
            {
                lblMsjCab.Text = objDistribPed.gError;
            }
